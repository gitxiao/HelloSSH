package com.cfrj.sppm.cfesf.dao;
...
import com.cfrj.sppm.cfesf.HibernateSessionFactory;
...
public abstract class BaseDao {
	...
	public Object save(Object transientInstance) throws Exception{
	...
	protected List fetchByHql(String hql) throws Exception{
		try {
	 ...
	 * @return
	 */
	protected Object findUniqueResult(String hql,Object[] paras) throws Exception{
		try {
			Query q= getSession().createQuery(hql);
	...
	@SuppressWarnings("unchecked")
	protected List fetchByHql(String hql, int firstResult, int maxResult) throws Exception{
		try {
			Query q= getSession().createQuery(hql);
...
//		findById();
	...
	@SuppressWarnings("unchecked")
	protected List fetchByHql(String hql, Object[] paras) throws Exception{
		try {
			Query q = getSession().createQuery(hql); 
			if(paras != null && paras.length > 0) {
				for(int i=0; i<paras.length; i++) {
...
//		jsonPara = Common.getDecoderUrl(jsonPara);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Object findById(Class cls, java.lang.Integer id) throws Exception{
		try {
			Session session = getSession();
